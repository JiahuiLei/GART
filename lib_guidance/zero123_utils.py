...
class Zero123(nn.Module):
    ...
    def refine(
        ...
        T = np.stack(
            [np.deg2rad(polar), np.sin(np.deg2rad(azimuth)), np.cos(np.deg2rad(azimuth)), radius],
            axis=-1,
        )
    ...
    def train_step(
        self, pred_rgb, polar, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False
    ):
        ...
        if as_latent:
            latents = (
                F.interpolate(pred_rgb, (32, 32), mode="bilinear", align_corners=False) * 2 - 1
            )
        else:
            pred_rgb_256 = F.interpolate(pred_rgb, (256, 256), mode="bilinear", align_corners=False)
            latents = self.encode_imgs(pred_rgb_256.to(self.dtype))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--radius", type=float, default=0, help="delta camera radius multiplier in [-0.5, 0.5]"
    )
    ...
    while True:
        outputs = zero123.refine(
            image, polar=[opt.polar], azimuth=[opt.azimuth], radius=[opt.radius], strength=0
        )
